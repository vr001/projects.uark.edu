...
CREATE TABLE IF NOT EXISTS Forum_Threads (
  ...
  thread_createdby_user_key INT NOT NULL,
  FOREIGN KEY (thread_createdby_user_key) REFERENCES Users(user_key),
  thread_deleted BIT(1) DEFAULT 0,
  thread_deleted_time TIMESTAMP,
  thread_deletedby INT,
  FOREIGN KEY (thread_deletedby) REFERENCES Users(user_key)
);
...
CREATE TABLE IF NOT EXISTS Forum_Messages (
  ...
  message_author_user_key INT NOT NULL,
  FOREIGN KEY (message_author_user_key) REFERENCES Users(user_key),
  message_deleted BIT(1) DEFAULT 0,
  message_deleted_time TIMESTAMP,
  message_deletedby INT,
  FOREIGN KEY (message_deletedby) REFERENCES Users(user_key),
  message_edited_time TIMESTAMP,
  message_editedby INT,
  FOREIGN KEY (message_editedby) REFERENCES Users(user_key)
);
